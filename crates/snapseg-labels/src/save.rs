//! Atomic, crash-safe write of a snapseg label to disk.
//!
//! Public surface: [`LabelDir::save`] + [`ProvenanceInputs`].
//! Implementation: validate dims → SHA-256 the source → pick a free
//! slot (sha8 with optional `-r{n}` region suffix) → stage every
//! artifact under `<id>.partial/` → atomic `fs::rename` to the final
//! `<id>/`. Best-effort cleanup of the partial dir on any failure.
...
mod tests {
    ...
    use crate::prompts::prompts_to_json;
    use crate::types::PromptSessionJson;
